...
static class Program
{
    ...
    static void Main(string[] args)
    {
        ...
        // ── 1. Find CuraEngine ──────────────────────────────────────────────
        var (curaExe, curaDef, curaExt, _) = FindValidCura();
        ...
        if (curaExe is null)
        {
            ...
            string reason = oldExe is not null
                ? $"CuraEngine {oldVersion} is installed but has a known bug that prevents slicing. " +
                  "Fabrium will now download and install Cura 5.12.0 (≈180 MB) alongside it."
                : "CuraEngine was not found on this PC. " +
                  "Fabrium will now download and install Cura 5.12.0 (≈180 MB) automatically.";
        ...
        if (!File.Exists(apiExe))
        {
            MessageBox.Show(
                ...
                "Fabrium — Startup Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            return;
        }

        var server = Process.Start(new ProcessStartInfo
        {
            FileName = apiExe, Arguments = "--urls http://*:5000",
        ...
        EnumerateCuraInstalls()
    {
        ...
        foreach (var root in roots)
        {
            ...
            try { dirs = Directory.GetDirectories(root, "UltiMaker Cura*")
            ...
            foreach (var dir in dirs)
            {
                ...
                results.Add((exe, def, ext, $"{major}.{minor}.{patch}", major, minor));
            }
        }
    ...
    static void KillOnPort(int port)
    {
        try
        {
            ...
            foreach (var line in output.Split('\n'))
            {
                ...
                { try { Process.GetProcessById(pid).Kill(entireProcessTree: true); } catch { } Thread.Sleep(500); }
            }
        }
        catch { }
    }
